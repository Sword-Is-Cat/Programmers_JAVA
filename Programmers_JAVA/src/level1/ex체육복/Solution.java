...
class Solution {
    public int solution(int n, int[] lost, int[] reserve) {
        ...
        //개개인의 체육복 소지여부를 나열하는 배열생성
        //도난:-1 / 통상:0 / 여분:+1 로 설정(여분+도난 중복은 +1-1=0으로 통상에 포함)
        ...
        for(int i = 0; i<n-1 ; i++) {
        	if(have[i]*have[i+1] == -1) {
        		...
        		have[i+1] = 0;
                i++;
                //인접한 두 학생이 각각 (1, -1), (-1, 1)인 경우에만 0, 0으로 변경
                //i번째 if문 ture일 시, have[i+1]=0 되어 i+1번째 if문 false 확정이므로 코드 최적화를 위해 i++
        	}
        }
